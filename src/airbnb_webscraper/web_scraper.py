...
class Context:
    ...
    def loadConfig(self):
        ...
        if config['custom_config']['location_label'] is None:
            self.location = input("Enter a label for the scraping location (eg: Edinburgh, Berlin): ")
        else:
            self.location = config['custom_config']['location_label']

        self.currency = config['custom_config']['currency']
    ...
    def UpdateContextWithHandlers(self, file_mgr, data_handler, session):
        """
...
class WebScraper:
    def __init__(self, context: Context):
        self.ctx = context
        self.mapTile_init = self.getMapTileList()
    ...
    def _trackDownloaded(self, listing_id):
        ...
        if listing_id not in self.downloaded_listingIDs:
            ...
            if len(self.downloaded_listingIDs) % 20 == 0:
    ...
    def _fetchInitialTile(self, coords):
        ...
        offset = 0
        url = f"{URLS[self.runner_type]}&currency={ctx.currency}"
        ...
        while offset < total_count:
            ...
            response = ( initial_response if offset == 0 else
                ctx.session.makeRequest('post',
                    f"{URLS[self.runner_type]}&currency={ctx.currency}",
    ...
    def _processListings(self, listings):
        ...
        for e in listings:
            ...
            if ctx.scrapeDescription:
                ...
                if ctx.translateDescriptionToEnglish:
...
if __name__ == '__main__':
    """
    Entry point for the Airbnb web scraping pipeline.
    This script performs a full end-to-end data pipeline
    
    1. **Setup**
        - Initializes context, logging, file, session and data managers
        - Retrieves location to scrape from config.toml, or requests it from user

    2. **Scraping**
        - Creates necessary working folders.
        - Runs the Airbnb scraping process in the specified location.
        - Each listing is saved as a JSON file.

    3. **Data Transformation**
        - Converts raw JSON files to structured CSVs (overview, calendar, pricing, etc.)

    4. **Cleanup & System Reboot**
        - Removes working folders
    """
    ...
    ctx.UpdateContextWithHandlers(file_mgr, data_handler, session)
    ...
    logger.info(f"Removing Airbnb chunks folder")
